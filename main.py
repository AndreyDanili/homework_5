...
singer_no_2020 = conn.execute("""SELECT s.name FROM singer s
...
album_genre = conn.execute("""SELECT a.name FROM album a
                            JOIN singer_album sa ON a.id = sa.album_id
                            JOIN singer s ON sa.singer_id = s.id
                            WHERE s.name IN ( 
...
singer_min_duration = conn.execute("""SELECT s.name, t.duration FROM singer s
                        JOIN singer_album sg ON s.id = sg.singer_id
...
album_min_track = conn.execute("""SELECT a.name FROM album a
                        WHERE a.id IN (
